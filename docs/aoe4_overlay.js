...
function getResourceLineAoE4(currentStep) {
  ...
  htmlString += getBOImageValue(
    gamePicturesFolder + 'unit_worker/villager.webp',
    currentStep,
    'villager_count',
    true
  );
  htmlString += getBOImageValue(
    gamePicturesFolder + 'building_economy/house.webp',
    currentStep,
    'population_count',
    true
  );
...
function evaluateBOTimingAoE4(timeOffset) {
  ...
  if (
    civilizationFlags['Dragon'] ||
    civilizationFlags['Zhu Xi'] ||
  ...
  for (const [currentStepID, currentStep] of enumerate(buildOrderData)) {
    ...
    if (civilizationFlags['Jeanne'] && !jeanneMilitaryFlag && nextAge > 1) {
      stepTotalTime += get_villager_time(civilizationFlags, currentAge); // one extra villager to create
