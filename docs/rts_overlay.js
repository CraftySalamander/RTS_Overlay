...
function activateVisualEditor() {
  ...
  if (visualEditortableWidgetDescription) {
    ...
    for (const element of elements) {
      initSelecWidgetImages(visualEditortableWidgetDescription, element.id, element.getAttribute('defaultValue'));
    }
  }
}
 ...
 *
 * @param {Array} tableDescription  Table with each option as [value, text, image].
...
class SinglePanelColumn {
  ...
  constructor(
    ...
    displayIfPositive = false, backgroundColor = null, textAlign = null, isSelectwidget = false) {
    // Check input types
    if (typeof field !== 'string' || (image && typeof image !== 'string') || (text && typeof text !== 'string') ||
    ...
    if (typeof italic !== 'boolean' || typeof bold !== 'boolean' ||
      ...
      typeof displayIfPositive !== 'boolean' || typeof isSelectwidget !== 'boolean') {
...
function getVisualEditorFromDescription(columnsDescription) {
  ...
  for (const column of columnsDescription) {
    const textImage = column.image ? getImageHTML(column.image, VISUAL_EDITOR_IMAGES_SIZE) : column.text;
    ...
    for (const column of columnsDescription) {
      ...
      if (column.isSelectwidget) {
        if (visualEditortableWidgetDescription) {
          ...
          htmlResult += ' defaultValue=' + currentStep[column.field] + '></select>'
