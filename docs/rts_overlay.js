...
function getImageHTML(
  ...
  // Add tooltip
  if (tooltipText) {
    imageHTML += '<span class="' + (tooltipOnLeft ? 'tooltiptext_left' : 'tooltiptext_right') + '">';
...
function getVisualEditor() {
  ...
  for (const [stepID, currentStep] of buildOrderData.entries()) {  // loop on all BO steps
    ...
    if (stepCount >= 2) {
      if (stepID >= 1) {
        htmlResult += getCircleButton(
          'icon/top_arrow.png', VISUAL_EDITOR_IMAGES_SIZE, 'move step up', false);
      }
    ...
    htmlResult += getCircleButton(
      'icon/blue_plus.png', VISUAL_EDITOR_IMAGES_SIZE, 'add step below', false);
    if (stepCount >= 2) {
      htmlResult += getCircleButton(
        'icon/red_cross.png', VISUAL_EDITOR_IMAGES_SIZE, 'delete this step', false);
    }
    htmlResult += '</td>';
